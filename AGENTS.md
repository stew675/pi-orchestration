...
| `runner/utils.ts` | Shared utilities: `notifyOrchestrator()` sends system messages to wake the orchestrator (with optional TUI visibility toggle); `notifyTuiOnly()` appends visual status entries without waking the model; `savePlanSafely()` wraps `StateManager.savePlan()` with shutdown guard. Also exports `buildFinalReviewMessage()` which assembles task summaries, artifact lists, and anti-redundancy instructions for the final review wakeup. |
...
- **Never modify plan.json directly** - always operate on `OrchestratorState.plan` in memory. Modifications will be automatically captured by the 1000ms debounced auto-save timer and persisted to disk safely. For immediate commits, call `StateManager.savePlan()`.
