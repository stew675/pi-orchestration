...
| `runner/utils.ts` | Shared utilities: `notifyOrchestrator()` sends system messages to wake the orchestrator (with optional TUI visibility toggle); `notifyTuiOnly()` appends visual status entries without waking the model. Also exports `buildFinalReviewMessage()` which assembles task summaries, artifact lists, and anti-redundancy instructions for the final review wakeup. |
...
- **`plans/plan.json`** - Read-only projection of the in-memory `OrchestratorState.plan`. This file is populated strictly from the disk copy during resumption after interruption then and ONLY then. No caching is maintained on this file; writes to disk are debounced via a 1000ms check timer that compares in-memory serialization and commits a safe deep-clone snapshot. Contains no `status` field, and instead contains an optional `attributes` string list tracking milestone events (e.g. `PLAN_APPROVED`, `CODE_REVIEW_APPROVED`, `VERIFIED`) used to perform stateless state inference on resumption.
...
- **Never modify plan.json directly** - always operate on `OrchestratorState.plan` in memory. All mutations (task CRUD, state transitions, status changes) are applied to this canonical in-memory object. The 1000ms debounced auto-save timer persists changes to disk automatically. No explicit flush is needed — the only manual save occurs at session shutdown.
