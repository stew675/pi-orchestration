...
| `context/prompts.ts` | Prompt templates. Exports minimal system prompts (`ORCHESTRATOR_PLANNING_SYSTEM_PROMPT`, `ORCHESTRATOR_EXECUTION_SYSTEM_PROMPT`) — the planning prompt includes the basic interactive flow (wait → explore → plan) so it works in its conversational loop, while detailed quality guidelines are injected contextually via `PLANNING_HINT_PRE_WRITE` on first write_plan call. Also exports `PLANNING_HINT_POST_WRITE` (stop-after-write reminder) and `PLANNING_HINT_EDIT` (thoroughness guidance, prepended to user edit feedback). Sub-agent prompt templates: task execution system prompt, validator system prompt, and summarizer system prompt. |
