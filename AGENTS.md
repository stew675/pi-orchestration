...
| File | Purpose |
|------|---------|
| `index.ts` | Extension entry point. Registers hooks (`session_start`, `before_agent_start`, `context`, `tool_result`, `turn_end`), commands, tools, and UI widget. Uses a **minimal system prompt + contextual hints** pattern for both planning and execution phases — the system prompt contains only absolute rules (role, constraints, stop-after-tool-call), while situational guidance is injected in-moment via `pi.sendMessage()` at key trigger points. The `before_agent_start` hook injects `PLANNING_HINT_ENTRY` on first turn of each planning session (one-shot). The `tool_result` hook injects `PLANNING_HINT_PRE_WRITE` on the first `orchestrate_write_plan` call (one-shot). Runs a **watchdog timer** (2-second interval during execution mode) that monitors for a stalled orchestrator - after 5 consecutive watchdog ticks with no sub-agent activity and no user pause flag, kicks the orchestrator with a nudge message. Resets whenever sub-agents are running, `_manualPause` is set, or the agent processes a turn. Disabled when `_manualPause` is set or planning mode is active. Also enforces **sub-agent idle timeout** and **max turns** limits via `enforceSubAgentLimits()` — iterates all registered agents, kills those exceeding thresholds via SIGTERM, sets `killedByWatchdog` reason for failure feedback. On `turn_end`, fires the **Accept/Edit dialog** (`showAcceptOrEditDialog`) when `_planJustUpdated` is true, giving the user 100 ms for tool output to render before overlaying the dialog.
...
| `context/prompts.ts` | Prompt templates. Exports minimal system prompts (`ORCHESTRATOR_PLANNING_SYSTEM_PROMPT`, `ORCHESTRATOR_EXECUTION_SYSTEM_PROMPT`) containing only absolute rules — all situational guidance is injected via contextual hints. Planning phase exports `PLANNING_HINT_ENTRY` (exploration strategy, sent on planning entry), `PLANNING_HINT_PRE_WRITE` (plan quality guidelines, sent on first write_plan call), `PLANNING_HINT_POST_WRITE` (stop-after-write reminder), and `PLANNING_HINT_EDIT` (thoroughness guidance, prepended to user edit feedback). Also defines sub-agent prompt templates: task execution system prompt, validator system prompt, and summarizer system prompt. |
