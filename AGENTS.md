...
| `core/state-machine.ts` | State transition validation. Defines `OrchestrationState` type (union of all valid states), `STATE_TRANSITIONS` map (current state → allowed next states), `transitionTo()` (validated state change with plan status sync), and predicates (`isActive()`, `isPlanningMode()`, `isExecutingMode()`). Also exports `mapPlanStatusToState()` for resume recovery. |
