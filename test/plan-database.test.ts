...
// ---------------------------------------------------------------------------
// async transaction — same semantics as sync but supports await
// ---------------------------------------------------------------------------

describe("transactionAsync", () => {
    it("commits mutations from an async callback", async () => {
        const db = PlanDatabase.empty();

        await db.transactionAsync(async (tx) => {
            tx.setGoal("Async goal");
            tx.addTask({ id: "task_phase1_async", description: "async task", complexity: "simple", timeoutMs: 720_000 });
        });

        expect(db.getGoal()).toBe("Async goal");
        expect(db.hasTask("task_phase1_async")).toBe(true);
    });

    it("rolls back on async callback error", async () => {
        const db = PlanDatabase.empty();

        await expect(
            db.transactionAsync(async (tx) => {
                tx.setGoal("Will fail");
                throw new Error("async abort");
            })
        ).rejects.toThrow("async abort");

        // State unchanged — goal is still empty
        expect(db.getGoal()).toBe("");
    });

    it("rolls back on validation failure in async transaction", async () => {
        const db = PlanDatabase.empty();
        ...
        ).rejects.toThrow(/Oversized tasks detected/i);
